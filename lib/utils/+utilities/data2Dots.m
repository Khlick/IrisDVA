function [values, densities, boundaryLine, sortOrder] = data2Dots(data, type, histArgs, kdeArgs, options)
  ...
  if ~isempty(nbins)
    ...
    elsefunction [values, densities, boundaryLine, sortOrder] = data2Dots(data, type, histArgs, kdeArgs, options)
  % data2Dots Create dot strip values for univariate data.
  % Histogram arguments are used for both discrete and continuous type. Setting
  % type to continuous will use the histogram to create bins and then fit the
  % points inside the kernel density estiamte.
  ...
  switch type
    ...
    case "continuous"
      ...
      % determine support
      if isempty(kdeArgs.Support)
        % supply calculation support as 10% increase in domain range
        %vDom = domain(values).';  
        %kdeArgs.Support = ([-1.05,1.05] ./ 2) .* diff(vDom) + mean(vDom);        
        kdeArgs = fastrmField(kdeArgs,"Support");
      end
      vDom = domain(values).';
      evPts = linspace(vDom(1),vDom(2),kdeArgs.NumPoints);
      kdeArgs = fastrmField(kdeArgs,"NumPoints");
      ...
      [pd, xi] = ksdensity(sortedData, evPts, 'Function', 'pdf', kArgCell{:});
  ...
  if ~isempty(options.DensityRange)
    ...
    % Ideally, boundaryLine will exist on options.DensityRange and pts will have
    % a range of: 
    % [
    %   options.DensityRange(1) + options.BaselineOffset, 
    %   options.DensityRange(2) - options.BaselineOffset
    % ]
    
    % by here, densities < bline.densities 
    totalDomain = domain(boundaryLine.densities(:)).';

    % first rescale to options.DensityRange
    dDen = linRescale( ...
    ...
    % now correct densities to be within the desired range with offsets
    dRes = linRescale( ...
      [densities(:); totalDomain(:)], ...
      options.DensityRange, ...
      [options.BaselineOffset,options.DensityOffset] ...
    );
    dRes(end-(0:1)) = [];
    ...
    pd_d = interp1(xi, boundaryLine.densities, values, 'linear').*0.975;
    correctionRange = max( ...
      [zeros(N, 1), rep(dOff(1), N), pd_d - dOff(1)], ...
      [], ...
      2 ...
    );
    dRes(dRes > pd_d) = correctionRange(dRes > pd_d);
    densities = dRes;
  end
...
function v = linRescale(d, newRange, offset)
  ...
  oldRange = [min(d), max(d)];
  newRange = newRange + [offset(1),-offset(2)];
  v = ( ...
    ( ...
...
function isValidRange(rg)
  ...
  switch type
    ...
    case "continuous"
      ...
      % supply calculation support as 10% increase in domain range
      vDom = domain(values).';
      support = vDom + [-0.05, 0.05] .* diff(vDom);
