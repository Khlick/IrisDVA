function saveName = ReadToSession(verbose)
  ...
  LS = iris.ui.Ticker();
  while (~LS.isready), end
  LS.update('Parsing files...');

  nf = size(files, 1);

  if nf > 1 && ~isempty(ver('parallel'))
    POOL = gcp();
  ...
  if nf == 1
    ...
    for I = 1:nf
      ...
      if accDataRead / totalDataSize < 1
        LS.updateAsPercent(accDataRead / totalDataSize);
      else
        LS.update('Loaded!', animate = true);
      end
