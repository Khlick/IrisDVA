...
function Data = getDataV2(grp)
  ...
  sourceLabels = arrayfun( ...
    @(src) ...
      cellfun( ...
        @(srcVal) ...
          h5readatt( fileName, srcVal, 'label' ), ...
        src.Value, ...
        'UniformOutput', false ...
        ), ...
    sourceLinks, ...
  ...
  sourceLinkAddr = [sourceLinks(:).Value];
  %}
  ...
  for gp = 1:numel(blockProps)
    ...
    thisBlock = epochGroups.Groups(gp);
  ...
  [epochResponses,epochBackgrounds,epochStimuli] = deal(cell(nEpochs,1));
  for ep = 1:nEpochs
    blk = epochBlocks(ep);
    
    % responses
    epochResponses{ep} = blk.Groups(contains({blk.Groups.Name}','/responses')).Groups;
    % devices
    epochBackgrounds{ep} = blk.Groups(contains({blk.Groups.Name}','/backgrounds'));
    % stimuli
    epochStimuli{ep} = blk.Groups(contains({blk.Groups.Name}','/stimuli'));
  end
  ...
  for rGroup = 1:nEpochs
    ...
    for cfg = 1:nConfigs
      ...
      paramLoc = epochStimuli{rGroup}.Groups(cfg).Groups( ...
        endsWith( ...
          {epochStimuli{rGroup}.Groups(cfg).Groups.Name}', ...
          '/parameters' ...
          ) ...
      ...
      theseAttributes = [ ...
        ...
        paramLoc.Attributes; ...
        epochStimuli{rGroup}.Groups(cfg).Attributes ...
        ];
    ...
    for cfg = 1:nBackgrounds
      ...
      paramLoc = epochBackgrounds{rGroup}.Groups(cfg);
