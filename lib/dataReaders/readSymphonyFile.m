function DATA = readSymphonyFile(fileName)
  ...
  if symphonyVersion < 2
    ...
    % if nexp > 1 let's collect and merge
    [Data,Meta,Notes] = deal(cell(1,nExperiments));
    for gg = 1:nExperiments
      Notes{gg} = getNotesV2(info.Groups(gg));
      Meta{gg} = getMetaV2(info.Groups(gg));
      Data{gg} = getDataV2(info.Groups(gg));
    end
    % merge notes into single entry
    Notes = cat(1,Notes{:});
    % get unique time entries
    [~,uqTimes] = unique(Notes(:,1));
    Notes = Notes(uqTimes,:);
    ...
    if nExperiments > 1
      if isequal(Meta{:})
        ...
        Meta = utilities.flattenStructs(Meta{:},uniquify=true);
      end
    end
  ...
  function Data = getDataV1()
    ...
    Data = {Data(sortInds)}; %return as cell
  end %data
